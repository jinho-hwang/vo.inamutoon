...
class Writer extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->load->view(CMS_VIEW_ROOT.'writer_View.php');
    ...
    public function Data_Load(){
        ...
        $count = $this->writer_m->all_count($tstr);

        $total_pages = ($count) ? ceil($count/$limit) : 0;
        $start = $limit * $page - $limit;

        $query = $this->writer_m->Data_Load($tstr,$start, $limit, $sidx, $sord,$searchString,$searchField,$searchOper);
        if (!empty($query)) {
            ...
            foreach ($query->result() as $row) {
                ...
                $response->rows[$i]['cell'] = array(
                   $row->code,$row->email,$row->wname,$row->tel,$row->mobile,$row->address,$row->isActive,$row->SKey,$row->regidate
                );
                $i++;
            }
        }
    ...
    public function Data_Add(){
        ...
        if (empty($post)) {
            ...
            if ($insert_id = $this->writer_m->Data_Add($post, $encrypt)) {
                $result = true;
                $message = 'Success';
            } else {
                $result = false;
    ...
    public function Data_Update(){
        ...
        if (empty($post)) {
            ...
            $this->load->model(CMS_MODEL_ROOT . 'writer_m');
            $affected_rows = $this->writer_m->Data_Update($post);
    ...
    public function Data_Delete(){
        ...
        if (empty($post)) {
            ...
            $this->load->model(CMS_MODEL_ROOT . 'writer_m');
            $affected_rows = $this->writer_m->Data_Del($post['id']);
