 <script>
        $(function(){
                ...
                if (returnData.result) {
                    $.ajaxFileUpload({
                        ...
                        error: function(data, status, e) {
                            ...
                            //alert(e);
                            $.ajax({
                                url: '<?echo(ROOT_URL);?>Sub_Cartoon/Load_ImgName_frame',
                                dataType: 'json',
                                method: 'post',
          ...
          $(function(){
            $("#list").jqGrid({
                mtype:"POST",
                url: '<?echo(ROOT_URL);?>Sub_Cartoon/Scene_Load/<?echo($pcode);?>/<?echo($cnum);?>',
                editurl: '<?echo(ROOT_URL);?>Sub_Cartoon/Scene_Update/<?echo($pcode);?>/<?echo($cnum);?>',
                datatype: "json",
                loadonce: false,
            ...
            $("#list").jqGrid('navGrid', '#pager', {
                ...
                closeAfterAdd: true,
                closeOnEscape: true,
                url: '<?echo(ROOT_URL);?>Sub_Cartoon/Scene_Add/<?echo($pcode);?>/<?echo($cnum);?>',
                afterSubmit: uploadFile,
                ...
                closeOnEscape: true,
                url: '<?echo(ROOT_URL);?>Sub_Cartoon/Scene_Delete',
    ...
    function View_Scene(val){
        window.open('<?echo(CONTENT_ASSETSROOT);?>/data/cartoon_scene2/mobile/' +val);
    }

    function free_View(){
        var URL = "<?echo(ROOT_URL);?>Sub_Cartoon/fView/<?echo($pcode);?>/<?echo($cnum);?>";
    ...
    function All_Delete(){
        if(window.confirm("전체 씬을 삭제하시겠습니까?")==true) {
            var URL = "<?echo(ROOT_URL);?>Sub_Cartoon/Scene_ADel/<?echo($pcode);?>/<?echo($cnum);?>";
    ...
    function File_Delete(){
        if(window.confirm("<?echo($title);?> > <?echo($cnum);?> 회차 전체를 삭제하시겠습니까?")== true){
            if(window.confirm("정말 삭제 하시겠습니까?")== true) {
                var URL = "<?echo(ROOT_URL);?>Main/FileDel/<?echo($pcode);?>/<?echo($cnum);?>";
    ...
    function New_Upload(){
        var URL = "<?echo(CONTENT_ROOT);?>/Cartoon/Scene_popup/<?echo($pcode);?>/<?echo($cnum);?>";
        popupOpen(URL, 450, 380, 100, 100);
    }


    function booking_View(){
        var URL = "<?echo(CONTENT_ROOT);?>Re_Cartoon/View/";
        window.open(URL);
    }
...
<style>
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">작품명:<?echo($title);?> > <?echo($cnum);?> 회차 > Scene 관리 </h1><div align="right"><input type="button" value="전체삭제"  onclick="javascrip:File_Delete();" />&nbsp;<input type="button" value="미리보기"  onclick="javascript:free_View();" />&nbsp;<input type="button" value="예약하기"  onclick="javascript:booking_View();" />&nbsp;<input type="button" value="용량축소"  onclick="javascript:free_View2();" />&nbsp;<input type="button" value="씬등록"  onclick="javascript:New_Upload();" /></div>
                <?if(empty($lastname)){?>
