...
if ( ! function_exists('Sub_Last_Name')) {
	function Sub_Last_Name($Sub_Rs)
	{
		if($Sub_Rs['sType']==0) {
			if (empty($Sub_Rs['sub_Title'])) {
				$name = $Sub_Rs['Cnt'] . '화';
			} else {
				$name = $Sub_Rs['sub_Title'];
			}
		}else if($Sub_Rs['sType']==1) {
			$name = '프롤로그';
		}else if($Sub_Rs['sType']==2) {
			$name = '휴재';
		}else if($Sub_Rs['sType']==3) {
			$name = '인터뷰';
		}else if($Sub_Rs['sType']==4) {
			$name = '특별편';
		}
...
if ( ! function_exists('fn_formatZero')) {
	function fn_formatZero($str,$maxNum)
	{
		return str_pad($str,$maxNum,"0",STR_PAD_LEFT);
	}
}
...
if ( ! function_exists('Curl')) {
	function Curl($target_url,$method,$post_data = null)
	{
		$data = array();
		if($method==1) {
			$curl = curl_init();
...
if ( ! function_exists('Make_AgeToDate')) {
	function Make_AgeToDate($age,$type)
	{

		$day = date('Y-m-d H:i:s');
		if ($type == 1) {
			$birth = date("Y-m-d", strtotime($day .  $age . "year"));
		}else if ($type == 2) {
			$birth = date("Y-m-d", strtotime($day .  $age . "month"));
		}else if ($type == 3) {
			$birth = date("Y-m-d", strtotime($day .  $age . "day"));
		}
...
if ( ! function_exists('Diff_Time')) {
	function Diff_Time($timeA,$type)
	{
		$cur_time = strtotime(date('Y-m-d H:i:s'));
		$ref_time = strtotime($timeA);
		$datetimediff = $cur_time - $ref_time;
		$timediff = $datetimediff % 86400;

		if($type=='H'){
			$result = floor($timediff / 3600);
		}else if($type=='M'){
			$result = floor($timediff % 3600 / 60);
		}else if($type=='S'){
			$result = floor($timediff % 3600 % 60);
		}
...
if ( ! function_exists('Print_Time')) {
	function Print_Time($d,$type){

		$tarr = date_parse_from_format("YmdHis",$d);
		...
		$rStr = $tarr['year'].$sp.$tarr['month'].$sp.$tarr['day'];
...
if ( ! function_exists('Print_age')) {
	function Print_age($birth){
		$birth_year = substr($birth, 0, 4);
		$birth_month = substr($birth, 4, 2);
		$brith_day = substr($birth, 6, 2);

		$tempStr = $birth_year.'.'.$birth_month.'.'.$brith_day;
...
if ( ! function_exists('Page_Link_Value'))
{
	function Page_Link_Value($page,$ViewCnt,$DataCnt)
	{
		($page==1) ? $LCnt = 0: $LCnt = ($page*$ViewCnt)-$ViewCnt;
		($DataCnt%$ViewCnt) > 0 ? $Max = (int)($DataCnt/$ViewCnt)+1 : $Max = ($DataCnt/$ViewCnt);

		$tempArr = array('Max'=>$Max,'List'=>$LCnt);
		return $tempArr;
	}
}
...
if ( ! function_exists('Short_Date'))
{
	...
	{
		$tempStr = String_Nomal_Left($date,10);
		return $tempStr;
	}
}

if ( ! function_exists('Secret_name'))
{
	function Secret_name($userid)
	{
		$tempArr = explode("@",$userid);
		$TLen = strlen($tempArr[0]);
		$TLen < 6 ? $dcnt = 3 : $dcnt = 6;

		$newStr = substr($tempArr[0], 0, $dcnt);
		for($i=0;$i<=($TLen-$dcnt)-1;$i++){
			$newStr .= '*';
		}
		return $newStr;
	}
...
if ( ! function_exists('dateDiff')) {
	function dateDiff($date1, $date2){
		$sdate = new DateTime($date1);
		$edate = new DateTime($date2);

		$dateinterval = date_diff($sdate,$edate);
		return $dateinterval->days;
	}
}

if ( ! function_exists('String_Right')) {
	function String_Right($value, $count)
	{
...
if ( ! function_exists('String_Left')) {
	function String_Left($string, $count)
	{
...
if ( ! function_exists('String_Nomal_Left')) {
	function String_Nomal_Left($string, $count)
	{
		$Tlen =mb_strlen($string, 'UTF-8');
		if($Tlen > $count) {
			$value = (mb_substr($string, 0, $count, "UTF-8"));
...
{
	// Alert 띄우기
	function confirm($msg = '', $url = '',$cancelurl ='')
	{
		$CI =& get_instance();
		if ( ! $msg) {
			$msg = '잘못된 접근입니다';
		...
		echo 'if(window.confirm("'. $msg.'") == true){';
		echo 'document.location.href="' . $url . '"';
		echo '}else{';
		if(!$cancelurl){
			echo 'window.location.reload();';
		}else {
			echo 'document.location.href="' . $cancelurl . '"';
		}
		echo '}</script>';
		exit;
	}
...
if ( ! function_exists('alert_refresh'))
{
	// Alert 후 창 닫음
	function alert_refresh($msg = '')
	{
		...
		echo '<meta http-equiv="content-type" content="text/html; charset=' . $CI->config->item('charset') . '">';
		echo '<script type="text/javascript"> alert("' . $msg . '"); window.opener.location.reload();</script>';
		exit;
	}
}
...
if ( ! function_exists('alert_parent_move_close'))
{
	// Alert 띄우기
	function alert_parent_move_close($msg = '', $url = '')
	{
		$CI =& get_instance();
		if ( ! $msg) {
			$msg = '잘못된 접근입니다';
		...
		echo '<meta http-equiv="content-type" content="text/html; charset=' . $CI->config->item('charset') . '">';
		echo '<script type="text/javascript">alert("' . $msg . '");';
		if ($url) echo 'window.opener.location.href="' . $url . '";';
		echo 'window.close();';
		echo '</script>';
		exit;
	}
