...
class Question_m extends CI_Model {
    ...
    public function all_count(){
        $sql = 'SELECT uid FROM tbl_order_question';
    ...
    public function Data_Load($start, $limit, $sidx, $sord,$searchString,$searchField,$searchOper){
        if($searchString!=''){
            if($searchOper=='eq'){
                $sql = "SELECT * FROM tbl_order_question a where ".$searchField."=? ORDER BY " . $sidx . " " . $sord . " LIMIT " . $start . ", " . $limit;
                $query = $this->db->query($sql,$searchString);
            }else if($searchOper=='cn'){
                $term = '%'.$searchString."%";
                $sql = "SELECT * FROM tbl_order_question a where ".$searchField." LIKE ? ORDER BY " . $sidx . " " . $sord . " LIMIT " . $start . ", " . $limit;
                $query = $this->db->query($sql,$term);
            }
        }else{
            $sql = "SELECT * FROM tbl_order_question a ORDER BY " . $sidx . " " . $sord . " LIMIT " . $start . ", " . $limit;
            $query = $this->db->query($sql);
        }
        return $query;
    }
    ...
    public function Data_Del($Bnum){
        ...
        $this->db->trans_start();
        $this->db->delete('tbl_order_question',$data);
    ...
    public function Update_Answer($Bnum){
        ...
        $this->db->update('tbl_order_question',$data);
        $affected_rows = $this->db->affected_rows();
